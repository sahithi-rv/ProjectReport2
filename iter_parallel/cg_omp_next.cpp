#include<bits/stdc++.h>
#include <chrono>
#include <omp.h>
#include <TAU.h>
#define PB push_back
#define MP make_pair
#define F first
#define S second
#define RI(a) scanf("%d",&a);
#define SZ(a) (int)(a.size())
#define SET(a,b) memset(a,b,sizeof(a))
#define TR(a,t) for(__typeof(a.begin()) t=a.begin();t!=a.end();t++)
#define rep(i,l,h) for(int i=(l); i<=(h);i++)
#define repd(i,h,l) for(int i=(h);i>=(l);i--)
#define ALL(a) a.begin(),a.end()
#define DRT()  int t; cin>>t; while(t--)
#define PRSNT(a,e) (a.find(e) != a.end())
#define MINH priority_queue<int, vector<int>, greater<int> >
#define output1(x) cout << #x << " " << x << endl;
#define output2(x,y) cout <<x<<" "<<y <<endl;
#define inp1(x) cin >> x;
#define inp2(x,y) cin >> x >> y;
#define MAX(a,b) a>b?a:b
#define MIN(a,b) a<b?a:b
#define N 1100
#define THREADS 4


using namespace std;
using namespace std::chrono;

int n;

float EPS = 0.0001;
void get_vectorB(char* fileB, float **b){
	ifstream myfile(fileB);
	rep(i,0,n-1){
		rep(j,0,n-1){
			myfile >> b[i][j];
		}
	}
	myfile.close();

}

void partial_matmul(float **b, float **out, int tid){
	// threads = 4
	int sq = sqrt(THREADS);
	int k = n/sq;
	//cout<<"k:" << k << endl;
	//cout << "partial_matmul" << tid << endl;
	if(tid == 0){
			//cout << "partial_matmul" << tid << endl;
		for(int i = 0;i<k;i++){
			for(int j = 0;j<k;j++){
				if(i==0 && j==0){
					out[i][j] = 4*b[i][j] - b[i][j+1] - b[i+1][j];
				}else if (i==0){
					out[i][j] = 4*b[i][j] - b[i+1][j] - b[i][j-1] - b[i][j+1];
				}else if (j==0){
					out[i][j] = 4*b[i][j] - b[i+1][j] - b[i-1][j] - b[i][j+1];
				}else{
					out[i][j] = 4*b[i][j] - b[i+1][j] - b[i-1][j] - b[i][j+1] - b[i][j-1];
				}
			}
		}
	}else if(tid == 1){
			//cout << "partial_matmul" << tid << endl;
		for(int i = 0;i<k;i++){
			for(int j = 1;j<=k;j++){
				if (i==0 && j==(k)){
					out[i][j-1] = 4*b[i][j] - b[i][j-1] - b[i+1][j];
				}else if(i==0){
					out[i][j-1] = 4*b[i][j] - b[i+1][j] - b[i][j-1] - b[i][j+1];
				}else if (j==(k)){
					out[i][j-1] = 4*b[i][j] - b[i-1][j] - b[i+1][j] - b[i][j-1];
				}else{
					out[i][j-1] = 4*b[i][j] - b[i+1][j] - b[i-1][j] - b[i][j+1] - b[i][j-1];
				}
			}
		}

	}else if(tid == 2){
			//cout << "partial_matmul" << tid << endl;

		for(int i = 1;i<=k;i++){
			for(int j = 0;j<k;j++){
				if (i==(k) && j==0){
					out[i-1][j] = 4*b[i][j] - b[i][j+1] - b[i-1][j];
				}else if (j==0){
					out[i-1][j] = 4*b[i][j] - b[i+1][j] - b[i-1][j] - b[i][j+1];
				}else if(i==k){
					out[i-1][j] = 4*b[i][j] - b[i][j-1] - b[i][j+1] - b[i-1][j];
				}
				else{
					out[i-1][j] = 4*b[i][j] - b[i+1][j] - b[i-1][j] - b[i][j+1] - b[i][j-1];
				}
			}
		}
	}else{
					//cout << "partial_matmul" << tid << endl;

		for(int i = 1;i<=k;i++){
			for(int j = 1;j<=k;j++){
				if(i==k && j==k){
					out[i-1][j-1] = 4*b[i][j] - b[i][j-1] - b[i-1][j];
				}else if(i==k){
					out[i-1][j-1] = 4*b[i][j] - b[i][j-1] - b[i][j+1] - b[i-1][j];
				}else if(j==k){
					out[i-1][j-1] = 4*b[i][j] - b[i-1][j] - b[i+1][j] - b[i][j-1];		
				}else{
					out[i-1][j-1] = 4*b[i][j] - b[i+1][j] - b[i-1][j] - b[i][j+1] - b[i][j-1];					
				}
			}
		}
	}
}

void matmul(float **b, float **out){
	rep(i,0,n-1){
		rep(j,0,n-1){
			if(i==0 && j==0){
				out[i][j] = 4*b[i][j] - b[i][j+1] - b[i+1][j];
			}else if (i==0 && j==(n-1)){
				out[i][j] = 4*b[i][j] - b[i][j-1] - b[i+1][j];
			}else if (i==(n-1) && j==0){
				out[i][j] = 4*b[i][j] - b[i][j+1] - b[i-1][j];
			}else if (i==(n-1) && j==(n-1)){
				out[i][j] = 4*b[i][j] - b[i][j-1] - b[i-1][j];
			}else if (i==0){
				out[i][j] = 4*b[i][j] - b[i+1][j] - b[i][j-1] - b[i][j+1];
			}else if (j==0){
				out[i][j] = 4*b[i][j] - b[i+1][j] - b[i-1][j] - b[i][j+1];
			}else if (i==(n-1)){
				out[i][j] = 4*b[i][j] - b[i][j-1] - b[i][j+1] - b[i-1][j];
			}else if (j==(n-1)){
				out[i][j] = 4*b[i][j] - b[i-1][j] - b[i+1][j] - b[i][j-1];
			}else{
				out[i][j] = 4*b[i][j] - b[i+1][j] - b[i-1][j] - b[i][j+1] - b[i][j-1];
			}
		}
	}
}

float dot_product(float **l, float **m ){

	float out = 0.0;
	#pragma omp parallel for reduction(+: out) num_threads(THREADS)
	rep(i,0,n-1){
		#pragma omp parallel for  shared(i,n) num_threads(THREADS)
		rep(j,0,n-1){
			out += l[i][j]*m[i][j];
		}
	}
	return out;
}

float dot_product(float **l ){

	float out = 0.0;

	#pragma omp parallel for reduction(+: out) num_threads(THREADS)
	rep(i,0,n-1){
		#pragma omp parallel for  shared(i,n) num_threads(THREADS)
		rep(j,0,n-1){
			out += l[i][j]*l[i][j];
		}
	}
	return out;
}

int main(int argc, char *argv[]){
	if(argc<2){
		printf("Enter  arguments: input file for vector B,  Grid size\n");
		return 0;		
	}
	n = atoi(argv[2]);
	cout << n << endl;
	//float r[n][n], b[n][n], z[n][n], p[n][n], x[n][n];
	float **r, **b, **z, **p, **x;
	
	b = (float **)malloc(sizeof(float*)*n);
	rep(i,0,n-1){
		b[i] = (float *)malloc((sizeof(float))*n);
	}
	r = (float **)malloc(sizeof(float*)*n);
	rep(i,0,n-1){
		r[i] = (float *)malloc((sizeof(float))*n);
	}
	z = (float **)malloc(sizeof(float*)*n);
	rep(i,0,n-1){
		z[i] = (float *)malloc((sizeof(float))*n);
	}
	p = (float **)malloc(sizeof(float*)*n);
	rep(i,0,n-1){
		p[i] = (float *)malloc((sizeof(float))*n);
	}	
	x = (float **)malloc(sizeof(float*)*n);
	rep(i,0,n-1){
		x[i] = (float *)malloc((sizeof(float))*n);
	}
	
	get_vectorB(argv[1], b);
	float del_new = 0,  del_old;
	//init b,p, r,x;
	srand((unsigned)time(0)); 

	
	for(int i = 0;i<n;i++){
   		for(int j = 0;j<n;j++){
   			//x[i][j] = 0.1*(rand()%10)+1;
   			x[i][j] = 0;
   		}
   	}

   	high_resolution_clock::time_point t1 = high_resolution_clock::now();

   	//cout << "lleling" << endl;
   	#pragma omp parallel for schedule(static) num_threads(THREADS)
	for(int i = 0;i<n;i++){
		#pragma omp parallel for schedule(static) shared(i,n) num_threads(THREADS)
   		for(int j = 0;j<n;j++){
   			r[i][j] = b[i][j];
   			p[i][j] = b[i][j];
   			
        	del_new += b[i][j]*b[i][j];
        }
    }

    del_old = del_new;
	float float1 = del_new;
	float float2 = del_old;
	float float3 = sqrt(abs(float1));
	float put;
    //iterations
    float vv, alpha, beta;
    float leps = log10(EPS);
	put = log10(float3/sqrt(float2) ) ;
	int sq = sqrt(THREADS);
	int row_sz = n/sq;
    int col_sz = n/sq;
    int iter = 0;

    while (put >= leps){

    	//matmul(p,z);
    	

    	int i, j;
    	#pragma omp parallel private( i,j) num_threads(THREADS)
    	{

    		//cout << "matmul" << endl;
    		int num_th = omp_get_num_threads();
    		//cout << num_th << endl;
    		int sq = sqrt(num_th);
    		int tid = omp_get_thread_num();
    		//cout <<  tid;
    		int row_sz = n/sq;
    		int col_sz = n/sq;
    		int row_start = (tid/sq)*row_sz;
    		int row_end = row_start + row_sz - 1;
    		int col_start = (tid%sq)*col_sz;
    		int col_end = col_start + col_sz - 1;

    		//cout << row_sz << " " << col_sz << endl;
    		//float inp[row_sz+1][col_sz+1], out[row_sz+1][col_sz+1];
    		TAU_PROFILE_TIMER(timers1, "allocation", "", TAU_USER);
    		TAU_PROFILE_START(timers1)
			float** inp = (float **)malloc(sizeof(float*)*(row_sz+1));

			rep(i,0,row_sz){
				inp[i] = (float *)malloc((sizeof(float))*(col_sz+1));
			}
			float **out = (float **)malloc(sizeof(float*)*(row_sz+1));
			rep(i,0,row_sz){
				out[i] = (float *)malloc((sizeof(float))*(col_sz+1));
			}
			TAU_PROFILE_STOP(timers1)
    		/*if(tid%sq == 1 && tid/sq == 1 ){
    			row_start++;
    			col_start++;
    		}else if(tid%sq == 1){
    			col_start++;
    		}else if(tid/sq == 1){
    			row_start++;
    		}*/

			//tried to make it all critical still got errors.
			//#pragma omp critical
			//{
			TAU_PROFILE_TIMER(timers2, "copy partial data", "", TAU_USER);
    		TAU_PROFILE_START(timers2)
	    		for(i = row_start;i<=row_end;i++){
	    			for(j = col_start;j<=col_end;j++){
	    				if(tid==0){
	    					inp[i-row_start][j-col_start] = p[i][j];
	    				}else if(tid == 1){
	    					inp[i-row_start][j-col_start+1] = p[i][j];
	    				}else if(tid==2){
	    					inp[i-row_start+1][j-col_start] = p[i][j];
	    				}else if(tid==3){
	    					inp[i-row_start+1][j-col_start+1] = p[i][j];
	    				}
	    			}
	    		}
	    	TAU_PROFILE_STOP(timers2)

			TAU_PROFILE_TIMER(timers3, "copy boundary data", "", TAU_USER);
    		TAU_PROFILE_START(timers3)
	    		//copy overlap
	    		if(tid==0){
	    			i = row_end+1;
					for(j = col_start;j<=col_end;j++){
						inp[i][j-col_start] = p[i][j];
					}
					j = col_end+1;
					for(i = row_start;i<= row_end;i++){
						inp[i-row_start][j] = p[i][j];
					}
				}else if(tid == 1){
					i = row_end+1;
					for(j = col_start;j<=col_end;j++){
						inp[i][j-col_start+1] = p[i][j];
					}
					j = col_start-1;
					for(i = row_start;i<=row_end;i++){
						inp[i-row_start][0] = p[i][j];
					}		
				}else if(tid==2){
					i = row_start-1;
					for(j = col_start;j<=col_end;j++){
						inp[0][j-col_start] = p[i][j];
					}
					j = col_end+1;
					for(i = row_start;i<= row_end;i++){
						inp[i-row_start+1][j] = p[i][j];
					}
				}else if(tid==3){
					i = row_start-1;
					for(j = col_start;j<=col_end;j++){
						inp[0][j-col_start+1] = p[i][j];
					}
					j = col_start-1;
					for(i = row_start;i<=row_end;i++){
						inp[i-row_start+1][0] = p[i][j];
					}
				}
				TAU_PROFILE_STOP(timers3)
			//cout << "copied: " << tid << endl;

				TAU_PROFILE_TIMER(timers4, "partial_matmul", "", TAU_USER);
    			TAU_PROFILE_START(timers4)
					int sq = sqrt(THREADS);
	int k = n/sq;
	//cout<<"k:" << k << endl;
	//cout << "partial_matmul" << tid << endl;
	if(tid == 0){
			//cout << "partial_matmul" << tid << endl;
		for(int i = 0;i<k;i++){
			for(int j = 0;j<k;j++){
				if(i==0 && j==0){
					out[i][j] = 4*inp[i][j] - inp[i][j+1] - inp[i+1][j];
				}else if (i==0){
					out[i][j] = 4*inp[i][j] - inp[i+1][j] - inp[i][j-1] - inp[i][j+1];
				}else if (j==0){
					out[i][j] = 4*inp[i][j] - inp[i+1][j] - inp[i-1][j] - inp[i][j+1];
				}else{
					out[i][j] = 4*inp[i][j] - inp[i+1][j] - inp[i-1][j] - inp[i][j+1] - inp[i][j-1];
				}
			}
		}
	}else if(tid == 1){
			//cout << "partial_matmul" << tid << endl;
		for(int i = 0;i<k;i++){
			for(int j = 1;j<=k;j++){
				if (i==0 && j==(k)){
					out[i][j-1] = 4*inp[i][j] - inp[i][j-1] - inp[i+1][j];
				}else if(i==0){
					out[i][j-1] = 4*inp[i][j] - inp[i+1][j] - inp[i][j-1] - inp[i][j+1];
				}else if (j==(k)){
					out[i][j-1] = 4*inp[i][j] - inp[i-1][j] - inp[i+1][j] - inp[i][j-1];
				}else{
					out[i][j-1] = 4*inp[i][j] - inp[i+1][j] - inp[i-1][j] - inp[i][j+1] - inp[i][j-1];
				}
			}
		}

	}else if(tid == 2){
			//cout << "partial_matmul" << tid << endl;

		for(int i = 1;i<=k;i++){
			for(int j = 0;j<k;j++){
				if (i==(k) && j==0){
					out[i-1][j] = 4*inp[i][j] - inp[i][j+1] - inp[i-1][j];
				}else if (j==0){
					out[i-1][j] = 4*inp[i][j] - inp[i+1][j] - inp[i-1][j] - inp[i][j+1];
				}else if(i==k){
					out[i-1][j] = 4*inp[i][j] - inp[i][j-1] - inp[i][j+1] - inp[i-1][j];
				}
				else{
					out[i-1][j] = 4*inp[i][j] - inp[i+1][j] - inp[i-1][j] - inp[i][j+1] - inp[i][j-1];
				}
			}
		}
	}else{
					//cout << "partial_matmul" << tid << endl;

		for(int i = 1;i<=k;i++){
			for(int j = 1;j<=k;j++){
				if(i==k && j==k){
					out[i-1][j-1] = 4*inp[i][j] - inp[i][j-1] - inp[i-1][j];
				}else if(i==k){
					out[i-1][j-1] = 4*inp[i][j] - inp[i][j-1] - inp[i][j+1] - inp[i-1][j];
				}else if(j==k){
					out[i-1][j-1] = 4*inp[i][j] - inp[i-1][j] - inp[i+1][j] - inp[i][j-1];		
				}else{
					out[i-1][j-1] = 4*inp[i][j] - inp[i+1][j] - inp[i-1][j] - inp[i][j+1] - inp[i][j-1];					
				}
			}
		}
	}
				TAU_PROFILE_STOP(timers4)

				TAU_PROFILE_TIMER(timers5, "accumulate_output", "", TAU_USER);
    			TAU_PROFILE_START(timers5)

				for(i = row_start;i<=row_end;i++){
					for(j = col_start;j<=col_end;j++){
						z[i][j] = out[i-row_start][j-col_start];
					}
				}
				TAU_PROFILE_STOP(timers5)
		
			#pragma omp barrier
    		free(inp);
			free(out);
			
			#pragma omp barrier

			#pragma omp single
			{
				vv=0.0;
			}
			#pragma omp barrier

    		TAU_PROFILE_TIMER(timer1, "dot_product1", "", TAU_USER);
    		TAU_PROFILE_START(timer1)
			#pragma omp for reduction(+: vv) collapse(2)  
			for(i = 0;i<n;i++){
				//#pragma omp parallel for  shared(i,n) num_threads(THREADS)
				for(j = 0;j<n;j++){
					vv += z[i][j]*p[i][j];
				}
			}
			TAU_PROFILE_STOP(timer1)			
			#pragma omp barrier
	    	#pragma omp single
	    	{
	    		alpha = del_old/vv;
	    	}
	    	#pragma omp barrier

    		TAU_PROFILE_TIMER(timer2, "vector_add1", "", TAU_USER);
    		TAU_PROFILE_START(timer2)
	    	#pragma omp for collapse(2)
			for(i = 0;i<n;i++){
				//#pragma omp parallel for schedule(static) shared(i,n) num_threads(THREADS)
				for(j = 0;j<n;j++){
					x[i][j] = x[i][j] + alpha*p[i][j];
				}
			}
			TAU_PROFILE_STOP(timer2)			
			#pragma omp barrier

    		TAU_PROFILE_TIMER(timer3, "vector_sub", "", TAU_USER);
    		TAU_PROFILE_START(timer3)
			#pragma omp for collapse(2) 
			for(i = 0;i<n;i++){
				//#pragma omp parallel for schedule(static) shared(i,n) num_threads(THREADS)
				for(j = 0;j<n;j++){
					r[i][j] = r[i][j] - alpha*z[i][j];
				}
			}
			TAU_PROFILE_STOP(timer3)			

			#pragma omp barrier
			
			#pragma omp single
			{
				del_new=0.0;
			}
			#pragma omp barrier
			
			TAU_PROFILE_TIMER(timer4, "dot_product2", "", TAU_USER);
    		TAU_PROFILE_START(timer4)
			#pragma omp for reduction(+: del_new) collapse(2)
			for(i = 0;i<n;i++){
				//#pragma omp parallel for  shared(i,n) num_threads(THREADS)
				for(j = 0;j<n;j++){
					del_new += r[i][j]*r[i][j];
				}
			}
			TAU_PROFILE_STOP(timer4)	

			#pragma omp barrier
			#pragma omp single
	    	{
	    		beta = del_new/del_old;
	    	}
	    	#pragma omp barrier

    		TAU_PROFILE_TIMER(timer5, "vector_add2", "", TAU_USER);
    		TAU_PROFILE_START(timer5)
	    	#pragma omp for collapse(2) 
			for(i = 0;i<n;i++){
				//#pragma omp parallel for schedule(static) shared(i,n) num_threads(THREADS)
				for(j = 0;j<n;j++){
					p[i][j] = r[i][j] + beta*p[i][j];
				}
			}
			TAU_PROFILE_STOP(timer5)			

			#pragma omp barrier

			#pragma omp single
	    	{
		    	del_old = del_new;
		    	float1 = del_new;
		    	float3 = sqrt(abs(float1));
				put = log10(float3/sqrt(float2) ) ;
		    	//cout << put << " ";
		    	iter++;
		    }
		    #pragma omp barrier


    	}
   }
    
    //cout << endl;
	high_resolution_clock::time_point t2 = high_resolution_clock::now();
    auto duration = duration_cast<microseconds>( t2 - t1 ).count();

    cout << "exec time:" << duration << endl;
    ofstream f;
    cout << "iterations: " << iter << endl;
    /*f.open("xtemp");
    rep(i,0,n-1){
    	rep(j,0,n-1){
    		f << std::fixed << std::setprecision(8) << x[i][j];
    		f << '\n';
    	}
    }
    f.close();*/
    free(b);
    free(r);
    free(z);
    free(x);
    free(p);
	

	

}